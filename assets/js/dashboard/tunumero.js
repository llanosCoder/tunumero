...
$(document).on('ready', function () {
    ...
    function obtenerFila(id, numero, estado, ate) {

        function calificarAtencion(cal) {
            $.post(
                url,
                {
                    op: 15,
            ...
            function (data) {
                if (data.success === 1) {
                    $.each(data.clientes, function (i, cliente) {
                        ...
                        ate = cliente.atencion_id;
                    });
                    if (numAtenciones > 0) {
                        obtenerFila(tipoAtencion, numero, est, ate);
                        $('.btn_volver').hide();
                    } else {
                        if (est === 1) {
                            estaEsperando(2);
            ...
            function (data) {
                if (data.success === 1) {
                    if (data.sesion.type !== 'cli') {
                        window.location.href = 'http://www.imasdgroup.cl/ejecutivos/clientes.html';
                    } else {
